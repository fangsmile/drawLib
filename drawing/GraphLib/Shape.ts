...
import { GraphLib, SizeConfig as RangeConfig } from "./GraphLib";
...
export class Shape extends Serializable {
    ...
    constructor(renderStyle: RenderStyle, rotateConfig: RotateConfig = null, regionConfig: RegionConfig = null, colorKey: string = null,isSetGraphLib:boolean=true) {
        super();
    ...
    * 子类均需重写
    ...
    prepareStyle(ctx: CanvasContext) {
        ...
        this.setLineDash(ctx, this.renderStyle.lineDash);
        ctx.lineJoin = this.renderStyle.lineJoin;
        ctx.lineCap = this.renderStyle.lineCap;
    }
    setLineDash(ctx: CanvasContext, line_dash: Array<number>) {
        let lineDash = []
        for (let i = 0; i < line_dash.length; i++) {
            lineDash[i] = line_dash[i] / ctx.scaleVal;
        }
    ...
    prepareHitStyle(hitCtx: CanvasContext) {
        ...
        hitCtx.lineWidth = (this.renderStyle.lineWidth > 0 ? this.renderStyle.lineWidth + 10 : this.renderStyle.lineWidth) / hitCtx.scaleVal;
        this.setLineDash(hitCtx, this.renderStyle.lineDash);
 ...
 */
    ...
    getPointRange(ctx?: CanvasContext) {
    ...
    //缩放元素
    scale(scale: number, zoomX: number = null, zoomY: number = null, sizeConifg: RangeConfig = null) {
        if ((zoomX != null && zoomX != undefined) && (zoomY != null && zoomY != undefined)) {//有定义缩放中心点
            var width = this.regionConfig.width * scale;
            var height = this.regionConfig.height * scale;
            if (sizeConifg && (width <= sizeConifg.minX || height <= sizeConifg.minY || width > sizeConifg.maxX && height > sizeConifg.maxY))
                return false;
            this.regionConfig.width = width;
            this.regionConfig.height = height;
            this.computeTranslate(scale, zoomX, zoomY);
        }
    }

    computeTranslate(scale: number, zoomX: number, zoomY: number) {
        this.regionConfig.x = (this.regionConfig.x - zoomX) * scale + zoomX;
        this.regionConfig.y = (this.regionConfig.y - zoomY) * scale + zoomY;
    }
